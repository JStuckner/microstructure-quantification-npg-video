...
def tv_chambolle(image_set, weight, max_stacking=None, eps=0.0002, verbose=False):
    """Performs chambolle filtering using the skimage library.

    :param image_set:
        Image array or 3D array containing time series of images
    :param weight:
        chambolle weight parameter
    :param eps:
        chambolle eps parameter
    :param max_stacking:
        maximum allowed stacking amount
    
    :return:
        denoised image or 3D array containing denoised images
    """

    from skimage.restoration import denoise_tv_chambolle # Chambolle denoising.
    from skimage.util import img_as_ubyte # Convert to 8-bit
    # Test if 2D or 3D and get shape of array
    try:
        rows, cols, num_frames = image_set.shape
    except ValueError:
        rows, cols = image_set.shape
    ...
    # If 2D image, denoise the single image
    if num_frames == 1:
        denoised = denoise_tv_chambolle(image_set, weight=weight, eps=eps)
        return img_as_ubyte(denoised)

    # If 3D and unlimited stacking is allowed
    elif (max_stacking is None or
          max_stacking < 1 or
          num_frames <= max_stacking):
        denoised = denoise_tv_chambolle(image_set, weight=weight, eps=eps)
        return img_as_ubyte(denoised)

    # If 3D image and stacking is limited
    else:
        # Initialize output array to zeros
        denoised = np.zeros((rows, cols, num_frames))

        # If no stacking is allowed
        if max_stacking == 1:
            for i in range(num_frames):
                denoised[:, :, i] = denoise_tv_chambolle(
                    image_set[:, :, i], weight=weight, eps=eps)
            return img_as_ubyte(denoised)

        # 3D image with limited stacking allowed
        else:
            # Number of images on either side of current image to stack.
            half_max = int(max_stacking / 2)
            # Need to adjust if the number of stacking images is even.
            if max_stacking % 2 == 0:
                # Use 1 less image from bottom half when stacking even number.
                adjust = 1 
            else:
                # Stack same number of images on either side for odd stacking.
                adjust = 0
            ...
            for i in range(num_frames):
                ...
                # In the middle of the dataset.  Stack an equal number of
                # images on either side of current image. If even stacking,
                # stack one less image from earlier in the dataset.
                else:
                    denoised[:, :, i] = denoise_tv_chambolle(
...
def old_tv_chambolle(image_set, weight, max_stacking=None, eps=0.0002, verbose=True):
    """Performs chambolle filtering using the skimage library.
    :param image_set:
    :param weight:
    :param eps:
    ...
    if num_frames == 1:
        ...
        if max_stacking == 1:
            ...
            half_stack = int(max_stacking - 1 / 2)
            for i in range(num_frames):
                if verbose:
                    if i % 10 == 0:
                        visualize.update_progress(i / num_frames)
