...
im_folder = r'E:\E_Documents\Research\NPG\Mechanical Testing\20171214 TEM tensile success\Movie 1\brittle clip\aligned'
in_folder = r'E:\E_Documents\Research\NPG\Mechanical Testing\20171214 TEM tensile success\Movie 1\brittle clip\segment_length'
mask = r'E:\E_Documents\Research\NPG\Mechanical Testing\20171214 TEM tensile success\Movie 1/brittle_mask.tif'
out_folder = r'E:\E_Documents\Research\NPG\Mechanical Testing\20171214 TEM tensile success\Movie 1\brittle clip\length_plot'
infile = 'fixstamps.txt'

files = inout.get_file_names(im_folder)
#files = files[:-60]
#files = files[:200]
times = []
...
for file in files:
    times.append((float(os.path.splitext(os.path.basename(file))[0][-5:])-8698)/30)
    try:
        im = misc.imread(file.replace(im_folder, in_folder), mode='L')
    except:
        print('skipping', file)
...
my_plot = ax.plot(times, l, lw=3)
plt.tight_layout()

for i, file in enumerate(files):
    #print(file)
    point = ax.plot(times[i],l[i], 'go', ms=15, c='orange')
    plt.savefig(file.replace(im_folder, out_folder), dpi=50)
    del ax.lines[1]
